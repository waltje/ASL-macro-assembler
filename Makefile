...
clean_unix: clean_doc
	...
	$(RM) $(ALLTARGETS) $(HOSTTARGETS) $(OBJDIR)*.dep $(TARG_OBJDIR)*.dep *.p $(TARG_OBJDIR)*.out $(TARG_OBJDIR)*.msh $(TARG_OBJDIR)*.rsc tests/testlog testlog

clean_windows:
	$(RM) *.dep *.msh *.rsc *$(HOST_OBJEXTENSION) *$(HOST_EXEXTENSION) *.res
...
bindist-zip: distdir
	-mv asl-$(VERSION)/lib/*.out asl-$(VERSION)/bin/
